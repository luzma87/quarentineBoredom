...
const GamePage = ({ firebase, authUser }) => {
    ...
    useEffect(() => {
        ...
        if (gameSession && authUser) {
            ...
            gameSession.currentGame.columns.forEach(col => {
    ...
    const onReady = () => {
        ...
        const mergedGameSession = { ...gameSession, currentGame: mergedCurrentGame };
    ...
    return (
        <div>
            ...
            <div>Current letter: {game.letter}</div>
            <div>
                {readyPlayers === 0 ? "No one's ready yet!" : null}
                {readyPlayers > 0 && readyPlayers < totalPlayers ? `${readyPlayers} players are ready!!` : null}
            ...
            <div>
                <table border="1">
                    ...
                    <tbody>
                        <tr>
                            {Object.entries(myStuff.columns).map(([colName, colContents], i) => {
                                return (
